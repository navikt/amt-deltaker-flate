...
export const lagOpplaringKategoriseringResponse = (
  ...
  return lagOpplaringKategoriseringMockRespons()
}

export const lagOpplaringKategoriseringDeltaljertRespons = (
  tiltakskode: Tiltakskode
): KodeverkResponse => {
  const base = detaljertKodeverkMockRespons.find(
    (k) => k.tiltakskode === tiltakskode
  ) ?? {
    tiltakskode,
    alternativer: [],
    sertifiseringValg: []
  }
  return berikMedValg(base, lagOpplaringKategoriseringMockRespons())
}
/**
 * Beriker en KodeverkResponse med valg fra en OpplaringKategorisering.
 * Setter valgt: true på alternativer som finnes i den flate kategoriseringen.
 */
const berikMedValg = (
  ...
  const valgteIder = new Set(
    kategorisering.valgteKategoriseringer.flatMap((k) =>
      k.valgteElementer.map((e) => e.id)
    )
  )
      ...
      if (container.type === KodeverkAlternativType.UTDANNING_GRUPPE) {
        return {
          ...
          utdanninger: container.utdanninger.map((u) => ({
            ...u,
            valgt: u.larefag.alternativer.some((v) => valgteIder.has(v.id)),
            larefag: {
              ...u.larefag,
              alternativer: u.larefag.alternativer.map((v) => ({
                ...v,
                valgt: valgteIder.has(v.id)
              }))
            }
          }))
        } satisfies KodeverkUtdanningGruppe
      }
      return container
    }),
    sertifiseringValg: kategorisering.valgteSertifiseringer
  }
}

/**
 * Basis kodeverk-data uten valg (alle alternativer har valgt: false).
 * Berikes med valg fra lagOpplaringKategoriseringMockRespons() ved oppslag.
 */
const detaljertKodeverkMockRespons: KodeverkResponse[] = [
  {
